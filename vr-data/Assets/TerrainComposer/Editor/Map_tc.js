	...
	function OnGUI()
	{
		...
		*/
		...
		if (global_script.map.button_parameters && global_script.map.key_edit) {
			...
			EditorGUI.DrawPreviewTexture(Rect(guiWidth3,gui_y3+45-scrollPos.y,799,44),global_script.tex2);
			
			GUI.color = Color.red;
			EditorGUI.LabelField(Rect(guiWidth3,gui_y3+45-scrollPos.y,position.width,50),"You need to create a free Bing key. Read the WorldComposer manual in the TerrainComposer folder how to do this.\nIf you are in Webplayer build mode, read the troubleshooting in the WorldComposer manual to get it working.\nAfter you entered the key, press F5 to refresh. Then press the 'K' button in Map Parameters to hide the key and this text.",EditorStyles.boldLabel);
			...
			{
				GUI.color = global_script.map.backgroundColor; 
				EditorGUI.DrawPreviewTexture(Rect(guiWidth3,gui_y3+(countKey*19.9)+220-96-(global_script.map.bingKey.Count*19)-scrollPos.y,global_script.get_label_width("Key"+countKey.ToString()+" -> '"+global_script.map.bingKey[countKey].key+"'",true),17),global_script.tex2);
				GUI.color = Color.red;
				EditorGUI.LabelField(Rect(guiWidth3,gui_y3+(countKey*19.9)+220-96-(global_script.map.bingKey.Count*19)-scrollPos.y,position.width,50),"Key"+countKey.ToString()+" -> '"+global_script.map.bingKey[countKey].key+"'",EditorStyles.boldLabel);
			}
...
//				wc_gui.x = 0;
		...
		{	
			...
			if (GUI.Button(wc_gui.getRect(1,0,25,16,true,false),"-",EditorStyles.miniButtonMid))
			{
				if (!current_area.export_heightmap_active) {
					--current_area.heightmap_zoom;
					GUI.changed = true;
			...
			GUI.color = global_script.map.color;
			EditorGUI.LabelField(wc_gui.getRect(1,12,50,19,true,false),"Manual");
			GUI.color = Color.white;
			...
			if (File.Exists(current_area.export_heightmap_path+"/"+current_area.export_heightmap_filename+".Raw") && fs == null) {
				if (GUI.Button(wc_gui.getRect(1,3,75,16,false,false),"Normalize")) {
				...
				if (global_script.map.warnings) {
					if ((area_size.x >= 16384 || area_size.y >= 16384) && (global_script.map.export_jpg || global_script.map.export_png)) {
						if (notify_text.IndexOf("The total") == -1) {
							...
							notify_text += "The total image size is bigger then 15k, read page 7 in the manual to understand the memory limitations.";
						}
					}
				}
			}
		...
		{	
				...
				settings_rect = Rect(0,gui_y,guiWidth2+1,219+gui_height);
...
//			current_area.converter_height = EditorGUI.FloatField(Rect(guiWidth2+204,102+gui_y2,200,17),current_area.converter_height);
	...
	function pull_elevation(ext_index: int) 
	{
		...
		if (global_script.map.elExt[ext_index].pull != null) {global_script.map.elExt[ext_index].pull.Dispose();}
	...
	function pull_image(ext_index: int)
	{
		...
		if (global_script.map.texExt[ext_index].pull != null) global_script.map.texExt[ext_index].pull.Dispose();
...
//			}
	...
	{
		return (height/(65535.0/9000.0))-100;
	}
}
